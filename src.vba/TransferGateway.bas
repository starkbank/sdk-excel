...
Public Function getListTransfers(cursor As String, listId As String, optionalParam As Dictionary)

    Dim baseUrl As String: baseUrl = "/v1/team/list/"
    Dim query As String
    Dim resp As response
    ...
    Set resp = StarkBankApi.getRequest(baseUrl + listId, query, New Dictionary)
    
    If resp.Status = 200 Then
        Set getListTransfers = resp.json()
    Else
        MsgBox resp.error()("message"), , "Erro"
    End If
End Function
